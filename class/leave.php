...
class Hrm_Leave
{
    ...
    public function update_holiday_data($data, $table, $format, $update_status, $post)
    {
    ...
    public function total_leave($start, $end, $work_in_week, $holiday)
    {
    ...
    public function leave_take($start, $end, $work_in_week, $holiday)
    {
    ...
    public function leave_remain($start, $end, $work_in_week, $holiday)
    {
        $start = date('Y-m-d', strtotime($start));
        $today = date('Y-m-d', time());
        $end = date('Y-m-d', strtotime($end));
    ...
    public function leave_type_form($field_value = null)
    {
    ...
    public function holiday($field_value = null)
    {
        $redirect = (isset($_POST['hrm_dataAttr']['redirect']) && !empty($_POST['hrm_dataAttr']['redirect'])) ? $_POST['hrm_dataAttr']['redirect'] : '';
        if ($field_value !== null) {
            $holiday['id'] = array(
    ...
    public function is_leave_take_prev($emp_id, $post_from, $post_to)
    {
        global $wpdb;
    ...
    public function count_leave_between_two_date($start_time, $end_time)
    {
    ...
    public function count_leave_exclude_holiday_weekend($start, $end, $work_in_week, $holiday)
    {
        $start = date('Y-m-d', strtotime($start));
        $end = date('Y-m-d', strtotime($end));
        if ($start > $end) {
    ...
    public function new_leave($post = null)
    {
        global $wpdb;
        $update       = false;
    ...
    public function send_new_leave_email($user_id, $post, $get_duration)
    {
    ...
    public function employee_to_employer_leave_message($post, $get_duration, $current_user)
    {
        ...
        ob_start(); ?>
            <div style="width: 600px; background: #eee; padding: 5px;">
                ...
                <tr>
                    ...
                    <td style="border: 1px solid #eee; font-size: 12px; padding: 10px;"><?php echo $post_from;
        _e(' to ', 'hrm');
        echo $post_to; ?></td>
                    <td style="border: 1px solid #eee; font-size: 12px; padding: 10px;"><?php echo $post['comment']; ?></td>
                    <td style="border: 1px solid #eee; font-size: 12px; padding: 10px;"><?php echo $this->leave_status($post['leave_status']);
        ; ?></td>
                    <td style="border: 1px solid #eee; font-size: 12px; padding: 10px;"><?php echo $post['apply_leave_total'];
        _e(' days', 'hrm'); ?></td>
                <tr>
                </table>
            </div>
    ...
    public function employer_to_employee_leave_message($send_to_user, $post, $get_duration, $current_user)
    {
        $post_from    = hrm_get_date2mysql($post['from']);
        $post_to      = hrm_get_date2mysql($post['to']);
        ob_start(); ?>
    ...
    public function individula_apply_leave($field_value = null)
    {
    ...
    public function assign($field_value = null)
    {
        $redirect = (isset($_POST['hrm_dataAttr']['redirect']) && !empty($_POST['hrm_dataAttr']['redirect'])) ? $_POST['hrm_dataAttr']['redirect'] : '';
    ...
    public function leave_status($status = null)
    {
        $leave = array(
            ''  => __('- Select -', 'hrm'),
            '1' => __('Pending', 'hrm'),
    ...
    public function leave_employer_status($status = null)
    {
        $leave = array(
            ''  => __('- Select -', 'hrm'),
            '2' => __('Cancel', 'hrm'),
    ...
    public function update_leave_status($postdata)
    {
        global $wpdb;
    ...
    public function leave_status_update_message($postdata, $prev_leave_row)
    {
        global $wpdb;
        $table  = $wpdb->prefix . 'hrm_leave';
    ...
    public function leave_status_update_message_body($get_apply_leave, $leave_owner, $status, $prev_leave_row)
    {
        ...
        ob_start(); ?>
            ...
            <table width="600" style="background: #fff; padding: 10px;">
                ...
                <tr>
                    <td style="padding: 10px;"><?php _e('You leave status is changes ', 'hrm');
        echo $prev_leave_satus;
        _e(' to ');
        echo $present_leave_satatus; ?></td>
                </tr>
            </table>
            <table  width="600" style="border-collapse: collapse; background: #fff; padding: 10px;">
            <thead>
        ...
        echo $post_to; ?></td>
                ...
                <td style="border: 1px solid #eee; font-size: 12px; padding: 10px;"><?php echo $leave_count;
        _e(' days', 'hrm'); ?></td>
            <tr>
            </table>
        <?php
    ...
    public function leave_emp_search_query($post, $limit, $pagenum)
    {
        $search_arg = array();
    ...
    public function leave_search_query($post, $users_id)
    {
        $search_arg = array();
