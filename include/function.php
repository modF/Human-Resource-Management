...
function hrm_user_can_access($page = null, $tab = null, $subtab = null, $access_point = null, $user_id = null)
{
    ...
    if ($subtab == null && user_can($user_id, $tab .'_'. $access_point)) {
        return true;
    } elseif ($subtab == null && ! user_can($user_id, $tab .'_'. $access_point)) {
        return false;
    }
...
function hrm_get_query_args()
{
    ...
    if (isset($_GET['tab']) && !empty($_GET['tab'])) {
        $tab = $_GET['tab'];
    } elseif (isset($menu[$page]) && is_array($menu[$page])) {
    ...
    if (isset($_GET['sub_tab']) && !empty($_GET['sub_tab'])) {
        $subtab = $_GET['sub_tab'];
    } elseif (isset($menu[$page][$tab]['submenu']) && count($menu[$page][$tab]['submenu'])) {
...
function hrm_result_limit()
{
    ...
    } elseif (isset($_REQUEST['hrm_attr']['limit']) && $_REQUEST['hrm_attr']['limit'] != '-1') {
